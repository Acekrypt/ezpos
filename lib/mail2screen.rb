...
module Mail2Screen
  def mail2html(mail, msg_id)
    ...
    ret << "<tbody>\n"
    ret << "  <tr><td class='label' nowrap='nowrap'>From:</td><td>#{address(mail.from_addrs, @msg_id)}</td></tr>\n"
    ret << "  <tr><td class='label' nowrap='nowrap'>To:</td><td>#{address(mail.to_addrs, @msg_id)}</td></tr>\n"
    if @mail.cc_addrs
      ret << "  <tr><td class='label' nowrap='nowrap'>CC:</td><td>#{address(mail.cc_addrs, @msg_id)}</td></tr>\n"
    end
    if @mail.bcc_addrs
      ret << "  <tr><td class='label' nowrap='nowrap'>BCC</td><td>#{address(mail.bcc_addrs, @msg_id)}</td></tr>\n"
    end
    ret << "  <tr><td class='label' nowrap='nowrap'>Subject:</td><td>#{h(mime_encoded?(mail.subject) ? mime_decode(mail.subject) : mail.subject)}</dd>\n"
    ret << "  <tr><td class='label' nowrap='nowrap'>Date:</td><td>#{h message_date(mail.date)}</td></tr>\n"
    if footer != ''
        ret << "  <tr><td class='label' nowrap='nowrap'>#{image_tag('mail/attachment')}</td><td>#{footer}</td></tr>\n"
    end
    ret << "  </tbody>\n"
    ret << "</table>\n"
        ...
        mail.parts.each { |part|
          if part.multipart?
            ...
            if part.content_type == "text/plain" or part.content_type.nil?
              ...
              ret << "<br/>Follows attached message:<hr/>" << mail2html(TMail::Mail.parse(part.body), msg_id)
            end
          end
        }
        return ret
  ...
  def add_to_contact(addr, msg_id)
    "&nbsp;<a href='/webmail/contacts/add_from_mail?cstr=#{CGI.escape(friendly_address(addr))}&retmsg=#{msg_id}'>Add to contacts</a>"
  end
