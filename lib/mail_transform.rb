...
class MailTransform
  ...
  def mail2html(mail, msg_id)
    ...
    ret = "-----Original Message-----\nFrom:#{address(mail.from_addrs, @msg_id)}\n"
    ret << "To:#{address(mail.to_addrs, @msg_id)}\n"
    if @mail.cc_addrs
      ret << "  CC:#{address(mail.cc_addrs, @msg_id)}\n"
    end
    if @mail.bcc_addrs
      ret << "BCC:#{address(mail.bcc_addrs, @msg_id)}\n"
    end
    ret << "Subject:#{mime_encoded?(mail.subject) ? mime_decode(mail.subject) : mail.subject}\n"
    ret << "Date:#{message_date(mail.date)}\n"
    ret << "\n"
    ret << "\n"
    ret << parsed_body
    ret << "\n"
