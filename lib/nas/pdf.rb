...
class PDF
    ...
    def AddPage(orientation='')
        bottom_line( 'Continued...' ) if @drawing_table # NAS
        ...
        draw_header if @drawing_table # NAS
    end
    ...
    def Link(x, y, w, h, link)
        ...
        @PageLinks[@page]=Array.new if @PageLinks[ @page ].nil?
    ...
    def putinfo
        out('/Producer '+textstring('NAS\'s PDF Writer '+FPDF_VERSION));
        unless @title.nil?
...
# NAS METHODS
   ...
   def SetLargeFont
    ...
    ROW_COLORS = [ [ 200, 200, 200 ], [ 255, 255, 255 ] ]

    def column_width( col )
        @widths[ col ]
    end

    def draw_cell( txt, column_num,align=nil )
        ...
        Cell( column_width( column_num ), 6, txt, 'LR', 0, align, 1 )
    end
...
#       @drawing_table=false
        @AutoPageBreak=false
        SetX( @table_xpos )
        Cell( table_width, (txt.empty? ? 0 : 5), txt, 'TLRB', 1, 'R' )
        @AutoPageBreak=true
#       @drawing_table=true
    end
    ...
    def set_row_style
        SetTextColor(0)
        if row_even?
            SetFillColor( *ROW_COLORS[0] )
        else
            SetFillColor( *ROW_COLORS[1] )
        end
    end
    ...
    def draw_header_cell( txt, col )
        Cell( column_width(col), 7, txt, 1, 0, 'C', 1)
    end

    def draw_header
        set_header_style
        0.upto( @header.length - 1) do |i|
            draw_header_cell( @header[i], i )
        end
        set_row_style
        Ln()
    end

    def DrawTable( header, widths, collection )
        raise 'Must have a width for each column' if header.size != widths.size
        @table_xpos=self.GetX
        @header=header
        @widths=widths
        @even=true
        @drawing_table=true
    ...
    def AddBookMark( txt, level=0, pos=nil)
        pos=self.GetY if pos.nil?
        @outlines.push( Hash[ 't'=>txt, 'l'=>level,'y'=>pos, 'p'=>self.PageNo ] )
    end
    ...
    def putbookmarks
        ...
        @outlines.each_index do | i |
            o=@outlines[i]
            self.newobj
            self.out( "<</Title #{textstring(o['t'])}" )
            self.out( "/Parent #{n+o['parent']} 0 R" )
            if o['prev']
                self.out "/Prev #{n+o['prev']} 0 R"
            end
            if o['next']
                self.out "/Next #{n+o['next']} 0 R"
            end
            if o['first']
                self.out "/First #{n+o['first']} 0 R"
            end
            if o['last']
                self.out "/Last #{n+o['last']} 0 R"
            end

            self.out( sprintf('/Dest [%d 0 R /XYZ 0 %.2f null]',1+2*o['p'],(@h-o['y'])*@k ) )
            self.out( '/Count 0>>')
            self.out( 'endobj' )
        end
        self.newobj
    ...
    def Rotate( angle, x=nil, y=nil )
        ...
        if angle != 0
            ...
            cx=x*@k;
            cy=(@h-y)*@k
            self.out(sprintf('q %.5f %.5f %.5f %.5f %.2f %.2f cm 1 0 0 1 %.2f %.2f cm',c,s,s*-1,c,cx,cy,cx*-1,cy*-1) );
        end
    end

    def SetDash(black=false,white=false)
        if black && white
            s=sprintf('[%.3f %.3f] 0 d',black*@k,white*@k)
        else
            s='[] 0 d'
        end
        self.out(s);
